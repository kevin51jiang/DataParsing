...
class DatabaseSender(DatabaseConnection):
    ...
    def insert_breadth(self, breadth_data):
        ...
        command += ("1" if breadth_data["humanities"] else "0") + ", " + ("1" if breadth_data["social_science"] else "0") + ", "
        command += ("1" if breadth_data["pure_science"] else "0") + ", " + ("1" if breadth_data["applied_science"] else "0") + "\n"
        # command += "FROM dual\n"
        command += "WHERE NOT EXISTS (\n" + not_exist + "\n)"
    ...
    def insert_course(self, course):
        ...
        command = "INSERT INTO " + self.course_table + " (course_code, course_abbr, course_number, course_id, " \
                                                       "course_name, credit, info, offering, is_online, prereqs," \
                                                       " coreqs, antireqs) \n"
        command += "SELECT '" + course.course_code + "', '" + course.course_abbr + "', " + re.sub("[^0-9]", "", course.course_number) + \
                   ...
                   course.info.replace("'", '"') + "', '" + ",".join(course.offering) + "', " + ("1" if course.online else "0") + \
